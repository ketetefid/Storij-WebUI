...
$(document).ready(function(){
    $("#sweepSeed").click(function(){
	// We want to sweep an existing seed.
	$("#initWalletModal").show();
	$("#initWalletInfo").html("<p> Enter the seed whose SpaceCash is to be swept into your wallet:</p>");
	// Calculating the best width of the text area based on the screen/parent width
	var optwidth=$("#initWalletInfo").width()-40;
	$("#initWalletInfo").append("<textarea spellcheck='false' id='existingSweepSeed' rows='5' style='width:"+optwidth+"px;'></textarea>");
	$("#initWalletFooter").html('<input type="button" value="Sweep" id="sweepTheSeed" class="OkCancelButton">');
    });

    // Starting the sweep
    $("#initWalletFooter").on('click','#sweepTheSeed',function(){ 
	var sweepSeed=$.trim($("#existingSweepSeed").val());
	if (sweepSeed=="") {
	    alert("Please enter the seed.");
	} else {
	    $("#initWalletInfo").html("<p>Wallet scanning has started. Please note that this will take some time to complete.</p>");
	    $("#initWalletFooter").html('<input type="button" value="OK" id="OkGotSeed" class="OkCancelButton">');
	    $.ajax ({
		url : "js/SetParams.php",
		cache : false,
		type : "POST",
		data : { sweepSeed : sweepSeed},
		headers : { 'auther': $('meta[name="auther"]').attr('content') }
	    });
	}
    });
    
    $("#viewSeed").click(function(){
	// We want to view the current seed.
	var isUnlocked = $("#unlockWallet").is(':checked');
	if (isUnlocked) {
	    $("#initWalletModal").show();
	    $("#initWalletInfo").html("<p> This is your current seed:</p>");
	    // Calculating the best width of the text area based on the screen/parent width
	    var optwidth=$("#initWalletInfo").width()-40;
	    $("#initWalletInfo").append("<textarea spellcheck='false' id='theCurrentSeed' rows='5' style='width:"+optwidth+"px;' readonly></textarea>");
	    $("#initWalletFooter").html('<input type="button" value="OK" id="OkGotSeed" class="OkCancelButton">');
	    $.ajax ({
		url : "js/SetParams.php",
		cache : false,
		type : "POST",
		data : { viewSeed : "viewSeed"},
		headers : { 'auther': $('meta[name="auther"]').attr('content') },
		success: function (theSeed) {
		    $("#theCurrentSeed").val(theSeed);
		    $("#theCurrentSeed").select();
		}
	    });
	} else {
	    alert("Please unlock your wallet to view the seed.");
	}
    });
});
...
$(document).ready(function(){
	...
	$('#SiaTable').html('<table id="wait4sia" class="t1"><tr><td><div class="HostConfigItems" style="justify-content:center">Please wait until HyperSpace daemon has started...<i class="fa fa-cog fa-spin fa-2x fa-fw"></i></div></td></tr></table>').show();
