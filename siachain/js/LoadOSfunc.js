...
$(document).ready(function(){
	...
	$("#FormatFlashModal").hide();
	    ...
	    headers : { 'auther': $('meta[name="auther"]').attr('content') },
	    success : function() {
		$("#DynInfoMessage").html('Reboot your device now to setup Sia on the attached drive.').show();
	    }
        });
    });				 
});
$(document).ready(function(){
	...
	if (/^[a-zA-Z0-9\-_/]*$/.test(siadirval) && /^\//.test(siadirval)) {
		...
		cache : false,
		headers : { 'auther': $('meta[name="auther"]').attr('content') },
		data: { siadir : "SCDIR", siadirval : siadirval}
	    });
	} else {
	    e.preventDefault();
...
$(document).ready(function(){
	...
	if (/^[0-9]*$/.test(rpc_portval) && !/^0/.test(rpc_portval)) {
		...
		cache : false,
		headers : { 'auther': $('meta[name="auther"]').attr('content') },
		data: { rpc_port : "sc_rpc_port" , rpc_portval : rpc_portval}
            });
	} else {
	    e.preventDefault();
...
$(document).ready(function(){
	...
	if (/^[0-9]*$/.test(host_portval) && !/^0/.test(host_portval)) {
		...
		cache : false,
		headers : { 'auther': $('meta[name="auther"]').attr('content') },
		data: { host_port : "sc_host_port" , host_portval : host_portval}
            });
	} else {
	    e.preventDefault();
...
$(function(){
	...
	$("#WebShellMessage").html('Stopping Sia daemon...<i class="fa fa-cog fa-spin fa-2x fa-fw">');
	$.ajax({
	    url : "js/SetParams.php",
	    type : "POST",
	    cache : false,
