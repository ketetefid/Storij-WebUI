...
if ( isset($_SESSION['uname']) and isset($_SESSION['authenticator']) and
  ...
  if(isset($_POST['unlockbyAutounlock'])){
    ...
    if ($lockbyautounlock==="true") {
      exec("sudo /etc/init.d/SCWalletUnlocker fstart");
    } else if ($lockbyautounlock==="false") {
      exec("sudo /etc/init.d/SCWalletUnlocker fstop");
    }
  }
  ...
  if (isset($_GET['newaddr'])) {
    ...
    $apiuser="";
    $apipass = trim(shell_exec('sudo /usr/local/bin/getapipass'));
    curl_setopt($chwa, CURLOPT_USERPWD, $apiuser . ":" . $apipass);    
  ...
  if (isset($_POST['viewSeed'])) {
    $apiwacurl = 'http://localhost:9980/wallet/seeds';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Sia-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['sweepSeed'])) {
    $apiwacurl = 'http://localhost:9980/wallet/sweep/seed';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Sia-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['storageSizes'])){
    ...
    $apiuser="";
    $apipass = trim(shell_exec('sudo /usr/local/bin/getapipass'));
    curl_setopt($chsh, CURLOPT_USERPWD, $apiuser . ":" . $apipass);    
  ...
  if (isset($_POST['announcing'])) {
    ...
    $apiuser="";
    $apipass = trim(shell_exec('sudo /usr/local/bin/getapipass'));
    curl_setopt($chha, CURLOPT_USERPWD, $apiuser . ":" . $apipass);    
