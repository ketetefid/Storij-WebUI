...
if ( isset($_SESSION['uname']) and isset($_SESSION['authenticator']) and
  ...
  if (isset($_POST['formatFlash'])) {
    ...
    if ($formatflashval==="true") {
      $bootstraptype=$_POST['bootStrapType'];
      $setupDev=$_POST['setupDev'];
      // Remove the word setup from the result
      $setupDevName=preg_replace("/setup/","",$setupDev);
      exec("udevadm info --query=all --name=/dev/$setupDevName | grep ID_SERIAL= | cut -d '=' -f 2 > /etc/SiaBerry/XSC-DevSetup");
      exec("../bin/SetParams 'setup_xsc' '$bootstraptype'");
  ...
  if(isset($_POST['hostname'])){
    ...
    exec("../bin/SetParams hostname '$hname'");
  }
  ...
  if (isset($_POST['startSiad'])) {
    ...
    if ($siadrequest==="true") {
      exec("sudo /etc/init.d/hsd start");
    } else if ($siadrequest==="false") {
      exec("sudo /etc/init.d/hsd stop");
    }
  }
  ...
  // Checking for a change to use_domain
  if (isset ($_POST['useDomain'])) {
      $usethedomainname=$_POST['useDomain'];
      if ($usethedomainname==="true") {
          exec("../bin/SetParams 'use_domain' '1'");
      } else if ($usethedomainname==="false") {
          exec("../bin/SetParams 'use_domain' '0'");
      }
  }
  ...
  if(isset($_POST['unlockbyAutounlock'])){
    ...
    if ($lockbyautounlock==="true") {
      exec("sudo /etc/init.d/XSCWalletUnlocker fstart");
    } else if ($lockbyautounlock==="false") {
      exec("sudo /etc/init.d/XSCWalletUnlocker fstop");
    }
  }
  // Checking for generating new address request  
  if (isset($_GET['newaddr'])) {
    $apiwacurl = 'http://localhost:5580/wallet/address';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
  ...
  if (isset($_POST['isWUnlocked'])) {
    $apiwscurl = 'http://localhost:5580/wallet';
    $chws = curl_init();
    curl_setopt($chws, CURLOPT_USERAGENT, "Hyperspace-Agent");
  ...
  if (isset($_POST['isScValid'])) {
    ...
    $apiwscurl = 'http://localhost:5580/wallet/verify/address/'.$destScAddr;
    $chws = curl_init();
    curl_setopt($chws, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chws, CURLOPT_URL, $apiwscurl);
    curl_setopt($chws, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['newSeed'])) {
    $apiwacurl = 'http://localhost:5580/wallet/init';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['existingSeed'])) {
    $apiwacurl = 'http://localhost:5580/wallet/init/seed';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  // Checking for viewing the current seed
  if (isset($_POST['viewSeed'])) {
    $apiwacurl = 'http://localhost:5580/wallet/seeds';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  // Checking for sweeping an existing seed
  if (isset($_POST['sweepSeed'])) {
    $apiwacurl = 'http://localhost:5580/wallet/sweep/seed';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
    ...
    $ExistingSeed=$_POST['sweepSeed'];
    $PostData=[ 'seed' => $ExistingSeed ];
    curl_setopt($chwa, CURLOPT_POSTFIELDS, $PostData);
  ...
  if (isset($_POST['destScAddr']) and isset($_POST['ScAmount'])) {
    $apiwacurl = 'http://localhost:5580/wallet/spacecash';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['lockWalletNoAuto'])) {
    $apiwacurl = 'http://localhost:5580/wallet/lock';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['unlockWalletNoAuto'])) {
    $apiwacurl = 'http://localhost:5580/wallet/unlock';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset ($_POST ['partList']) || isset ($_POST ['devFormatList'])) {
    ...
    $file='/boot/SiaBerry/XSCHostPoints.txt';
  ...
  if (isset($_POST['storageSizes'])){
    ...
    $hosts=file('/boot/SiaBerry/XSCHostPoints.txt');
	...
	$apihscurl = 'http://localhost:5580/host/storage/folders/add';
	...
	foreach ($storagefolders as $storagefolder) {
	  if (strcmp($hostpath,$storagefolder->path)==0) {
	    ...
	    $apihscurl = 'http://localhost:5580/host/storage/folders/resize';
	    $storageApiData=[ 'path' => $hostpath, 'newsize' => $hostsize ];
	    break 1;
	  }
	}
	$chsh = curl_init();
	curl_setopt($chsh, CURLOPT_USERAGENT, "Hyperspace-Agent");
	...
	foreach ($storagefolders as $storagefolder) {
	  if (strcmp($hostpath,$storagefolder->path)==0) {
	    $apihscurl = 'http://localhost:5580/host/storage/folders/remove';
  ...
  if (isset($_POST['SetSettings'])) {
    ...
    $apiwacurl = 'http://localhost:5580/host';
    $chwa = curl_init();
    curl_setopt($chwa, CURLOPT_USERAGENT, "Hyperspace-Agent");
    curl_setopt($chwa, CURLOPT_URL, $apiwacurl);
    curl_setopt($chwa, CURLOPT_HEADER, false);
  ...
  if (isset($_POST['announcing'])) {
    // The announcing part
    $apihacurl = 'http://localhost:5580/host/announce';
    $chha = curl_init();
    curl_setopt($chha, CURLOPT_USERAGENT, "Hyperspace-Agent");
