<?php
$apicurl = 'http://localhost:5580/consensus';
$chc = curl_init();
curl_setopt($chc, CURLOPT_USERAGENT, "Hyperspace-Agent");
...
$apiwurl = 'http://localhost:5580/wallet';
$chw = curl_init();
curl_setopt($chw, CURLOPT_USERAGENT, "Hyperspace-Agent");
...
if ($httpCode == 200) {
  ...
  $hastings=$dataw->confirmedspacecashbalance;
  $scoins=floatval($hastings)/pow(10,24);
  $scoinsout=floatval($dataw->unconfirmedoutgoingspacecash)/pow(10,24);
  $scoinsin=floatval($dataw->unconfirmedincomingspacecash)/pow(10,24);
  
  #$sfunds=$dataw->siafundbalance; HyperSpace has removed siafunds.
  $sfunds="None";
}

$apiwaurl = 'http://localhost:5580/wallet/addresses';
$chwa = curl_init();
